...
export default function ProfileGameGrid({ logs }: { logs: GameLog[] }) {
  ...
  // Paginate list so rendering 500+ items doesn't freeze the DOM
              ...
              <div className="aspect-[3/4] w-full bg-slate-950 relative overflow-hidden">
                {log.coverUrl ? (
                  <img
                    ...
                    onError={(e) => {
                      // Fallback image if CDN fails
                      (e.target as HTMLImageElement).src = `https://cdn.cloudflare.steamstatic.com/steam/apps/${log.externalGameId}/header.jpg`;
                      /*
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 pt-4">
          <button
            ...
            className="px-3 py-1.5 bg-slate-900 border border-slate-800 text-xs font-bold text-slate-300 rounded-lg hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Previous
          </button>
